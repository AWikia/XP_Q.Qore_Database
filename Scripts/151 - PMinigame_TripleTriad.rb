...
class TriadCard
  ...
  def self.createBack(type=-1,noback=false)
    ...
    if type>=0
      ...
      typerect=Rect.new(64*0,type*28,64,28)
...
# End the scene here
  ...
  def pbStartScene(battle)
    ...
    @viewport.z=99999
    addBackgroundPlane(@sprites,"background",getDarkModeFolder+"/triad_bg",@viewport)
...
def pbBuyTriads
  ...
  for i in 1..PBSpecies.maxValue
    if $Trainer.owned[i]
      ...
      commands.push([price,speciesname,_INTL("{1} - ${2}",speciesname,price.to_s_formatted),i])
    end
  end
      ...
      if Input.trigger?(Input::C)
        ...
        if quantity>0
          ...
          if Kernel.pbConfirmMessage(_INTL("I can pay ${1}. Would that be OK?",price.to_s_formatted))
            ...
            Kernel.pbMessage(_INTL("Turned over the {1} card and received ${2}.",itemname,price.to_s_formatted))
            commands=[]
            for i in 0...$PokemonGlobal.triads.length
              item=$PokemonGlobal.triads[i]
              speciesname=PBSpecies.getName(item[0])
