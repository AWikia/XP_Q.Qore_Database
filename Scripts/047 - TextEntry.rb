...
=end
  ...
  @@Characters0=[ # Latin
    [("ABCDEFGHIJÅÂÊ"+"KLMNOPQRSTÝËÎ"+"UVWXYZÇÐÃÑÕÞÆ"+"ÁÉÍÓÚÀÈÌÒÙÄÖÜ"+"COŠŽŒGDEIUÔÛL").scan(/./),_INTL("UPPER")],
    [("abcdefghijåâê"+"klmnopqrstýëî"+"uvwxyzçðàñõþæ"+"áéíóúàèìòùäöü"+"cošžœgdeiuôûl").scan(/./),_INTL("lower")],
    [("??G????T?????"+"?????S??F??O?"+"aß?de??????µ?"+"???p?st?f????"+"?????????????").scan(/./),_INTL("greek")],
...
=begin
  @@Characters1=[ # ??????
     ...
     [("aß?de?????   "+"?µ???p?st?   "+"f???         "+"???????????  "+"             ").scan(/./),_INTL("lower")],
     [(",.:;!?ß€£??§µ"+"\"'()<>[]¿,¥®©"+"~@#%*&$ÿ      "+"+-=^_/\\|     "+"0123456789   ").scan(/./),_INTL("other")],
  ]
  @@Characters2=[ # Kirynla
     [("?????????????"+"?????????????"+"?????????????"+"?????????????"+"???????????? ").scan(/./),_INTL("UPPER")],
