...
def pbItemEditorNew(defaultname)
  ...
  if itemname=="" && !defaultname
    ...
    Kernel.pbMessage(
       _ISPRINTF("Put the item's graphic ({1:03d}.png or {2:s}.png) in Graphics/Items, or it will be blank.",
...
class ItemNameProperty
  def set(settingname,oldsetting)
    message=Kernel.pbMessageFreeText(_INTL("Set the value for {1}.",settingname),
       oldsetting ? oldsetting : "",false,20)
  end
...
def pbItemEditor
  ...
  items=[
     ...
     [_INTL("Purchase price"),LimitProperty.new(999999),
     ...
     [_INTL("Use Out of Battle"),EnumProperty.new([
         ...
         _INTL("TM"),_INTL("HM"),_INTL("On Pokémon reusable"),_INTL("TD")]),
    ...
    if cmd[0]==1   # Swap dex up
      ...
      if cmd[1]==0   # Add new dex
        cmd2=Kernel.pbMessage(_INTL("Fill in this new Dex?"),
           [_INTL("Leave blank"),_INTL("Fill with National Dex"),_INTL("Fill from file"),_INTL("Cancel")],4)
        if cmd2==0 || cmd2==1 || cmd2==2
          newdex=[nil]
          if cmd2==1
          ...
          if cmd2==2 # NEW Addition
            if safeExists?("RegionalDex.txt")
              File.open("RegionalDex.txt","r"){|f|
                 lineno=1
                 f.each_line {|line|
                   text=line
                   text.gsub!(/\n/,"")
                   id=getID(PBSpecies,text)
                   newdex[lineno]=id
                   lineno+=1
                 }
              }
            else
              File.open("RegionalDex.txt","wb"){|f|
                f.write('')
              }
              Kernel.pbMessage(_INTL("RegionalDex.txt was not found and created an empty dex instead."))
            end
          end
        ...
        cmd2=Kernel.pbMessage(_INTL("\\ts[]Do what with this Dex?"),
            [_INTL("Edit"),_INTL("Replace"),_INTL("Copy"),_INTL("Delete"),_INTL("Cancel")],5)
        if cmd2==0
          regionallist[cmd[1]-1]=pbRegionalDexEditor(regionallist[cmd[1]-1])
          refreshlist=true
        elsif cmd2==1 # NEW Addition
          cmd3=Kernel.pbMessage(_INTL("Fill in this new Dex?"),
             [_INTL("Leave blank"),_INTL("Fill with National Dex"),_INTL("Fill from file"),_INTL("Cancel")],4)
          if cmd3==0 || cmd3==1 || cmd3==2
            newdex=[nil]
            if cmd3==1
              for i in 1...numDexDatas
                newdex[i]=i
