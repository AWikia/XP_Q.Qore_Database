...
class VoltorbFlip
  ...
  def pbNewGame
    ...
    for i in 0...25
      ...
      @squares[i]=[(i-x).abs*64+128+64,(i/5).abs*64,squareValues[r],false]
  ...
  def pbCreateSprites
    ...
    for i in 0...3
      for j in 0...25
        ...
        icons[j]=[@directory+"tiles",@squares[j][0],@squares[j][1],320+64+(i*64)+(points*64),0,64,64]
      end
      icons.compact!
    ...
    @cursor[0]=[@directory+"cursor",0+128+64,0,0,0,64,64]
  end

  def getInput
    if Input.trigger?(Input::UP)
      pbPlayCursorSE() # Was pbSEPlay("Choose")
      if @index[1]>0
        @index[1]-=1
        @sprites["cursor"].y-=64
      else
      ...
      if @cursor[0][3]==64 # If in mark mode
        ...
        for i in 0...@marks.length+1
          if @marks[i]==nil
            @marks[i]=[@directory+"tiles",@index[0]*64+128+64,@index[1]*64,256,0,64,64]
          elsif @marks[i][1]==@index[0]*64+128+64 && @marks[i][2]==@index[1]*64
            @marks.delete_at(i)
            @marks.compact!
        ...
        for i in 0...@squares.length
          if @index[0]*64+128+64==@squares[i][0] && @index[1]*64==@squares[i][1] && @squares[i][3]==false
            ...
            if @squares[i][2]==0
              ...
              animation=[]
              for j in 0...3
                animation[0]=icons[0]=[@directory+"tiles",@index[0]*64+128+64,@index[1]*64,704+(64*j),0,64,64]
              ...
              animation=[]
              for j in 0...6
                animation[0]=[@directory+"explosion",@index[0]*64-32+128+64,@index[1]*64-32,j*128,0,128,128]
                pbDrawImagePositions(@sprites["animation"].bitmap,animation)
                pbWait(3)
                @sprites["animation"].bitmap.clear
              end
              ...
              animation=[]
              for j in 0...4
                animation[0]=[@directory+"flipAnimation",@index[0]*64-14+128+64,@index[1]*64-16,j*92,0,92,96]
                pbDrawImagePositions(@sprites["animation"].bitmap,animation)
                pbWait(3)
                @sprites["animation"].bitmap.clear
              end
...
#          end
      ...
      pbPlayDecisionSE() # Was pbSEPlay("Choose")
      @sprites["cursor"].bitmap.clear
      if @cursor[0][3]==0 # If in normal mode
        @cursor[0]=[@directory+"cursor",128+64,0,64,0,64,64]
      ...
      elsif Kernel.pbConfirmMessage(_INTL("If you quit now, you will recieve {1} Coin(s). Will you quit?",@points.to_s_formatted))
        Kernel.pbMessage(_INTL("{1} received {2} Coin(s)!",$Trainer.name,@points.to_s_formatted))
        $PokemonGlobal.coins+=@points
        @points=0
        pbUpdateCoins
        @sprites["curtain"].opacity=0
  ...
  def pbUpdateRowNumbers(num,voltorbs,i)
    ...
    for j in 0...2
      @numbers[j]=[@directory+"numbersSmall",472+j*16+64,i*64+8,numImages[j].to_i*16,0,16,16]
    end
    @voltorbNumbers[i]=[@directory+"numbersSmall",488+64,i*64+34,voltorbs*16,0,16,16]
    # Display the numbers
  ...
  def pbUpdateColumnNumbers(num,voltorbs,i)
    ...
    numImages=numText.split(//)[0...2]
    for j in 0...2
      @numbers[j]=[@directory+"numbersSmall",(i*64)+152+j*16+64,328,numImages[j].to_i*16,0,16,16]
    end
    @voltorbNumbers[i]=[@directory+"numbersSmall",(i*64)+168+64,354,voltorbs*16,0,16,16]
    # Display the numbers
    pbDrawImagePositions(@sprites["numbers"].bitmap,@numbers)
    pbDrawImagePositions(@sprites["numbers"].bitmap,@voltorbNumbers)
  end
  ...
  def pbAnimateTile(x,y,tile)
    ...
    for i in 0...3
      points=tile if i==2
      icons[i]=[@directory+"tiles",x+64,y,320+(i*64)+(points*64),0,64,64]
