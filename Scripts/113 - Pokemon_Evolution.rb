...
# Starts the evolution screen with the given Pokemon and new Pokemon species.
  ...
  def pbUpdateNarrowScreen
    if @bgviewport.rect.y<10*4
      @bgviewport.rect.height-=2*4
      if @bgviewport.rect.height<Graphics.height-32
        @bgviewport.rect.y+=4
...
# Opens the evolution screen
  def pbEvolution(cancancel=true,prism=false)
    ...
    if prism
      Kernel.pbMessageDisplay(@sprites["msgwindow"],
         _INTL("\\se[]What?\r\n{1} is prisming!\\^",@pokemon.name)) { pbUpdate }
    else
      Kernel.pbMessageDisplay(@sprites["msgwindow"],
    ...
    if canceled
      ...
      @pokemon.resetTemperature # Reset Temperature after evolving
      # Resets form to 0 for some species
      @pokemon.form = 0 if (isConst?(@pokemon.species,PBSpecies,:MOTHIM) ||
                            isConst?(@pokemon.species,PBSpecies,:ANT1) ||
                            isConst?(@pokemon.species,PBSpecies,:MAKTV) ||
                            isConst?(@pokemon.species,PBSpecies,:DIGEA))
      ...
      Kernel.pbReceiveTrophy(:TEVOLVER)
    end
  end
end
...
def pbMiniCheckEvolution(pokemon,evonib,level,poke)
  ...
  when PBEvolution::Happiness
    return poke if pokemon.happiness>=160
  when PBEvolution::HappinessDay
    return poke if pokemon.happiness>=160 && PBDayNight.isDay?
  when PBEvolution::HappinessNight
    return poke if pokemon.happiness>=160 && PBDayNight.isNight?
  when PBEvolution::HappinessMoveType
    if pokemon.happiness>=160 # 220
      for i in 0...4
  ...
  when PBEvolution::TypeInParty
    for i in $Trainer.party
      return poke if !i.isEgg? && i.hasType?(level)
    end
