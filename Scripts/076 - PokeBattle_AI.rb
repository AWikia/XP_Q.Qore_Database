...
##### Alter score depending on the move's function code ########################
    ...
    when 0x49
      ...
      score+=30 if opponent.pbOwnSide.effects[PBEffects::Reflect]>0 ||
                   ...
                   opponent.pbOwnSide.effects[PBEffects::AuroraVeil]>0 || # changed added
    ...
    when 0x68
      if opponent.effects[PBEffects::Substitute]>0 ||
        ...
        score-=90 if isConst?(opponent.ability,PBAbilities,:RKSSYSTEM)
    ...
    when 0xD5, 0xD6,0x178,0x190,0x207
    ...
    when 0x159
      score+=5 if opponent.stages[PBStats::ATTACK]>0
      score+=5 if opponent.stages[PBStats::DEFENSE]>0
      score+=5 if opponent.stages[PBStats::SPEED]>0
      score+=5 if opponent.stages[PBStats::SPATK]>0
      score+=5 if opponent.stages[PBStats::SPDEF]>0 
      if skill>=PBTrainerAI.mediumSkill
        hasdamagingattack=false
        for thismove in attacker.moves
          if thismove.id!=0 && thismove.basedamage>0
    ...
    when 0x160
      if opponent.effects[PBEffects::Substitute]>0 ||
         ...
         isConst?(opponent.ability,PBAbilities,:RKSSYSTEM)
        score-=90
      elsif opponent.pbHasType?(:NORMAL)
        score-=90
      end
    when 0x161
      if opponent.effects[PBEffects::Substitute]>0 ||
         ...
         isConst?(opponent.ability,PBAbilities,:RKSSYSTEM)
        score-=90
      elsif opponent.pbHasType?(:JELLY)
        score-=90
      end
    when 0x164
    when 0x165
      score+=10 if !attacker.pbTooHigh?(PBStats::DEFENSE) && opponent.hp<=(opponent.totalhp/4)
      score+=10 if !attacker.pbTooHigh?(PBStats::EVASION) && opponent.hp<=(opponent.totalhp/4)
    when 0x166
      if opponent.pbHasType?(:SHARPENER)
        score-=90
      end
    when 0x167
      if opponent.pbHasType?(:MAGIC)
        score-=90
      end
    when 0x168
      if opponent.pbHasType?(:LAVA)
        score-=90
      end
    when 0x170
      reserves=attacker.pbNonActivePokemonCount
      foes=attacker.pbOppositeOpposing.pbNonActivePokemonCount
      if pbCheckGlobalAbility(:NERVOUSCRACK)
        score-=100
      elsif skill>=PBTrainerAI.mediumSkill && reserves==0 && foes>0
        score-=100 # don't want to lose
      elsif skill>=PBTrainerAI.highSkill && reserves==0 && foes==0
    ...
    when 0x171
      score+=attacker.stages[PBStats::DEFENSE]*5
      if move.basedamage==0
        if attacker.pbTooHigh?(PBStats::EVASION)
          score-=45
        else
          score-=attacker.stages[PBStats::EVASION]*10
          if skill>=PBTrainerAI.highSkill
            aspeed=pbRoughStat(attacker,PBStats::EVASION,skill)
            ospeed=pbRoughStat(opponent,PBStats::EVASION,skill)
            if aspeed<ospeed && aspeed*2>ospeed
              score+=30
            end
          end
        ...
        score+=10 if attacker.stages[PBStats::EVASION]<0
      end
    when 0x172, 0x195, 0x196, 0x197
      score-=100 if !isConst?(attacker.species,PBSpecies,:KOULUNDIN)
    when 0x173
      score+=attacker.stages[PBStats::EVASION]*5
      if move.basedamage==0
        if attacker.pbTooHigh?(PBStats::DEFENSE)
          score-=45
        else
          score-=attacker.stages[PBStats::DEFENSE]*10
          if skill>=PBTrainerAI.highSkill
            aspeed=pbRoughStat(attacker,PBStats::DEFENSE,skill)
            ospeed=pbRoughStat(opponent,PBStats::DEFENSE,skill)
            if aspeed<ospeed && aspeed*2>ospeed
              score+=30
            end
          end
    ...
    when 0x174
      score+=20 if opponent.hp<=(opponent.totalhp/4)
    when 0x175
      score-=90 if opponent.effects[PBEffects::Khleri]>0
    when 0x176
      if opponent.effects[PBEffects::Substitute]>0 ||
         isConst?(opponent.ability,PBAbilities,:MULTITYPE) ||
         isConst?(opponent.ability,PBAbilities,:RKSSYSTEM)
        score-=90
      elsif opponent.pbHasType?(:CHLOROPHYLL)
        score-=90
      end
    when 0x179
    when 0x180
    when 0x181
      if !opponent.pbCanReduceStatStage?(PBStats::ATTACK,attacker) &&
         !opponent.pbCanReduceStatStage?(PBStats::DEFENSE,attacker) &&
         !opponent.pbCanReduceStatStage?(PBStats::SPATK,attacker) &&
         !opponent.pbCanReduceStatStage?(PBStats::SPDEF,attacker) &&
         !opponent.pbCanReduceStatStage?(PBStats::SPEED,attacker)
        score-=100
      elsif attacker.pbNonActivePokemonCount()==0
        score-=100 
      else
        score+=(opponent.stages[PBStats::ATTACK]*4)
        score+=(opponent.stages[PBStats::DEFENSE]*4)
        score+=(opponent.stages[PBStats::SPATK]*4)
        score+=(opponent.stages[PBStats::SPDEF]*4)
        score+=(opponent.stages[PBStats::SPEED]*4)
        score-=(attacker.hp*100/attacker.totalhp)
      end
    when 0x182,0x183
      if opponent.pbTooLow?(PBStats::ATTACK) &&
         ...
         opponent.pbTooLow?(PBStats::ACCURACY) &&
         opponent.pbTooLow?(PBStats::EVASION)
        score-=90
      else
        avstat=0
        avstat+=opponent.stages[PBStats::ATTACK]
    ...
    when 0x186
      score-=40 if !attacker.pbTooHigh?(PBStats::ATTACK)
      score-=40 if !opponent.pbTooLow?(PBStats::ATTACK)
    when 0x187
      score-=90 if opponent.level>attacker.level  # Don't prefer the move if opponent's level is grather than attacker's
      addition=5
      addition+=attacker.level+opponent.level
      score+=addition if attacker.level>opponent.level
    when 0x188
      evtotal=(attacker.ev[0]+attacker.ev[1]+attacker.ev[2]+attacker.ev[3]+attacker.ev[4]+attacker.ev[5])/18      
      score-=90 if evtotal<1  # Don't prefer the move otherwise
      score+=evtotal if evtotal>0 # The higher the ev yield, the higher are the chances of preferring the move
    when 0x189
      score-=40 if !attacker.pbTooHigh?(PBStats::SPATK)
      score-=40 if !opponent.pbTooLow?(PBStats::SPATK)
    when 0x192
      if !@doublebattle
        score-=100
      elsif attacker.pbPartner.isFainted?
    ...
    when 0x198
      ...
      if attacker.hp==attacker.totalhp || factor>8
        score-=90
      else
        score+=100
        score-=factor*10
    ...
    when 0x199
      burn=attacker.pbHasType?(:FIRE) || attacker.pbHasType?(:LAVA)
      score-=90 if !burn
    when 0x200
      score-=100 if !isConst?(attacker.species,PBSpecies,:JOICON)
    when 0x201
    when 0x202
    ...
    when 0x216
    when 0x218
      # TODO: Score Lower if poisoned
      score-=45 if opponent.pbTooLow?(PBStats::SPEED)
    when 0x219
      score-=90 if opponent.status==0
    when 0x220
    when 0x222
      score+=20 if attacker.stages[PBStats::DEFENSE]>0
    when 0x233 # changed added for aurora veil
      score-=90 if attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0
    when 0x234
      score-=90 if opponent.effects[PBEffects::ThroatChop]>0
    when 0x235 
      score-=90 if attacker.effects[PBEffects::LaserFocus]>0
    when 0x236
    when 0x237
    when 0x238
      if move.basedamage==0
        if opponent.pbTooLow?(PBStats::EVASION)
          score-=90
        else
          score+=40 if opponent.turncount==0
          score+=opponent.stages[PBStats::EVASION]*10
        end
      else
        score+=10 if opponent.turncount==0
        score+=20 if opponent.stages[PBStats::EVASION]<0
      end
    when 0x239, 0x268, 0x284, 0x293
      attspeed=pbRoughStat(attacker,PBStats::SPEED,skill)
      oppspeed=pbRoughStat(opponent,PBStats::SPEED,skill)
      score+=30 if attspeed>oppspeed
    when 0x240
=begin
      score+=20 if PBMoveData.new(opponent.effects[PBEffects::TwoTurnAttack]).function==0x14D || # Phantom Force
...
=end
    ...
    when 0x243
      score-=40 if !attacker.pbTooHigh?(PBStats::SPEED)
      score-=40 if !opponent.pbTooLow?(PBStats::SPEED)
    when 0x244
    when 0x245
      score+=50 if opponent.effects[PBEffects::DoomElist]
    when 0x246
      score+=50 if opponent.effects[PBEffects::FlashFire]
    when 0x247
      if opponent.effects[PBEffects::Substitute]>0 ||
         isConst?(opponent.ability,PBAbilities,:MULTITYPE) ||
         isConst?(opponent.ability,PBAbilities,:RKSSYSTEM)
        score-=90
      elsif opponent.pbHasType?(:DOOM)
        score-=90
      end
    when 0x248
    ...
    when 0x255
    when 0x256
      score-=100 if !isConst?(attacker.species,PBSpecies,:MORPEKO)
    when 0x263
      if opponent.pbTooHigh?(PBStats::ATTACK) &&
         opponent.pbTooHigh?(PBStats::SPATK)
        score-=90
    ...
    score-=90 if !didsomething
    when 0x265
      if opponent.effects[PBEffects::Substitute]>0 ||
         isConst?(opponent.ability,PBAbilities,:MULTITYPE) ||
         isConst?(opponent.ability,PBAbilities,:RKSSYSTEM)
        score-=90
      elsif opponent.pbHasType?(:PSYCHIC)
        score-=90
      end
    when 0x267
    ...
    when 0x271
      ...
      score-=90 if attacker.hp<=sublife
    when 0x272
=begin
      changeside=false
      for i in 0...2
...
=end
    ...
    when 0x275
      if opponent.effects[PBEffects::Substitute]>0 ||
         ...
         isConst?(opponent.ability,PBAbilities,:RKSSYSTEM)
        score-=90
      elsif opponent.pbHasType?(:ELECTRIC)
        score-=90
      end
    when 0x276
    when 0x277
   ...
   when 0x281
      leaf=attacker.pbHasType?(:GRASS) || attacker.pbHasType?(:CHLOROPHYLL)
      score-=90 if !leaf
    when 0x282
    when 0x283
      attspeed=pbRoughStat(attacker,PBStats::SPEED,skill)
      oppspeed=pbRoughStat(opponent,PBStats::SPEED,skill)
      score-=90 if oppspeed>attspeed ||
                   opponent.effects[PBEffects::SilveryBliss]
    when 0x285
      score-=90 if attacker.pbOwnSide.effects[PBEffects::Electromania]>0
    when 0x286
      score-=90 if attacker.pbOwnSide.effects[PBEffects::Brainologic]>0
    when 0x287
      # Intentionally no need of AI
    when 0x288
      # TODO
    when 0x289
      score-=90 if attacker.pbOwnSide.effects[PBEffects::Fierymania]>0
    when 0x290
    ...
    if move.basedamage>0
      ...
      if typemod==0 || score<=0
        ...
        score=0
            ...
            opponent.hasWorkingAbility(:HERBLOBBY)
        score=0
        ...
        score=0
      elsif skill>=PBTrainerAI.mediumSkill && isConst?(move.type,PBTypes,:DOOM) &&
            opponent.hasWorkingAbility(:DOOMELIST)
        score=0
      elsif skill>=PBTrainerAI.mediumSkill && isConst?(move.type,PBTypes,:GRASS) &&
            opponent.hasWorkingAbility(:LONGGRASS)
        score=0
...
################################################################################
  ...
  def pbBetterBaseDamage(move,attacker,opponent,skill,basedamage)
    ...
    when 0x6F # Psywave
      basedamage=attacker.level
    when 0x70, 0x202, 0x252, 0x278 # OHKO
  ...
  def pbRoughDamage(move,attacker,opponent,skill,basedamage)
    ...
    if (attacker.hasWorkingItem(:NORMALBOX) && isConst?(type,PBTypes,:NORMAL)) ||
       ...
       damagemult=(basedamage*1.5).round
    end
    ...
    if (attacker.hasWorkingItem(:NORMALGEM) && isConst?(type,PBTypes,:NORMAL)) ||
       ...
       (attacker.hasWorkingItem(:BLIZZARDEM) && isConst?(type,PBTypes,:BLIZZARD)) ||
    ...
    # Long Grass
    if skill>=PBTrainerAI.highSkill
      if attacker.hasWorkingAbility(:LONGGRASS) &&
         attacker.effects[PBEffects::LongGrass] && isConst?(type,PBTypes,:GRASS)
        atk=(atk*1.5).round
      end
    end
    # Slow Start
    ...
    if skill>=PBTrainerAI.bestSkill
      if opponent.hasWorkingAbility(:FLUFFY) &&
         move.isContactMove? && !attacker.hasWorkingAbility(:LONGREACH)
