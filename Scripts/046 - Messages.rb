...
def pbGetGoldString
  ...
  if $Trainer
    ...
    if $data_system.respond_to?("words")
      moneyString=_INTL("{1} {2}",$game_party.gold.to_s_formatted,$data_system.words.gold)
    else
      moneyString=_INTL("{1} {2}",$game_party.gold.to_s_formatted,Vocab.gold)         
    end
  end
  return moneyString
end
...
def Kernel.pbMessageDisplay(msgwindow,message,letterbyletter=true,commandProc=nil)
  ...
  textchunks=[]
  controls=[]
  while text[/(?:\\([WwFf]|[Ff][Ff]|[Tt][Ss]|[Cc][Ll]|[Mm][Ee]|[Ss][Ee]|[Ww][Tt]|[Ww][Tt][Nn][Pp]|[Cc][Hh]|[Gg])\[([^\]]*)\]|\\([Gg]|[Cc][Nn]|[Ww][Dd]|[Ww][Mm]|[Oo][Pp]|[Cc][Ll]|[Ww][Uu]|[\.]|[\|]|[\!]|[\x5E])())/i]
    ...
    for i in 0...controls.length
      if controls[i] && controls[i][2]<=msgwindow.position && msgwindow.waitcount==0
        ...
        when "g" # Display gold window
          ...
          if param=="1"
            goldwindow=pbDisplayLinkBalanceWindow(msgwindow)
          else
