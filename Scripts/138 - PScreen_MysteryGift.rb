...
MYSTERYGIFTURL = "https://www.dropbox.com/home/MysteryGift.txt"
...
# QQC Edit
      if safeExists?("MysteryGift.txt")
        string=IO.read("MysteryGift.txt")
      else
        string=""
      end
  # QQC Edit end
  ...
  if gift[1]==0
    ...
    if $PokemonBag.pbCanStore?(gift[2],gift[1])
      ...
      if $ItemData[item][ITEMUSE]==3 || $ItemData[item][ITEMUSE]==4
        Kernel.pbMessage(_INTL("\\me[ItemGet]{1} received \\c[1]{2}\\c[0]!\\nIt contained \\c[1]{3}\\c[0].\\wtnp[30]",
           $Trainer.name,itemname,PBMoves.getName($ItemData[item][ITEMMACHINE])))
      elsif isConst?(item,PBItems,:LEFTOVERS)
        Kernel.pbMessage(_INTL("\\me[ItemGet]{1} received some \\c[1]{2}\\c[0]!\\wtnp[30]",$Trainer.name,itemname))
      elsif qty>1
        Kernel.pbMessage(_INTL("\\me[ItemGet]{1} received {2} \\c[1]{3}\\c[0]!\\wtnp[30]",$Trainer.name,qty,itemname))
      else
        Kernel.pbMessage(_INTL("\\me[ItemGet]{1} received one \\c[1]{2}\\c[0]!\\wtnp[30]",$Trainer.name,itemname))
      end
      $Trainer.mysterygift[index]=[id]
      return true
    end
