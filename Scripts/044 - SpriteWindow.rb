...
def isDarkBackground(background,rect=nil)
  ...
  #return (r+g+b)/3<128
  # return (r+g+b)/3 < 128
  # return (r*0.299+g*0.588+b*0.110)<114
...
#      Otherwise, the Ftext is left aligned.
#  4 - Base color
#  5 - Shadow color
...
# _viewport_ is a viewport to place the background in.
def addBackgroundOrColoredPlaneGlobal(sprites,planename,background,color,viewport=nil)
  bitmapName=pbResolveBitmap("Graphics/Global Pictures/#{background}")
  if bitmapName==nil
    # Plane should exist in any case
...
class Window_CommandPokemon < Window_DrawableCommand
  ...
  def drawItem(index,count,rect)
    ...
    pbDrawShadowText(self.contents,rect.x,rect.y,rect.width,rect.height,
       @commands[index], (index == self.index) ? getHighlightColor[0] : self.baseColor,(index == self.index) ? getHighlightColor[1] : self.shadowColor)
  end
end
