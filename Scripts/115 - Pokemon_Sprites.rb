...
class PokemonSprite < SpriteWrapper
  ...
  def setSpeciesBitmap(species,female=false,form=0,shiny=false,shadow=false,back=false,egg=false,box=false,color=0)
    @_iconbitmap.dispose if @_iconbitmap
    @_iconbitmap=pbLoadSpeciesBitmap(species,female,form,shiny,shadow,back,egg,box,color)
    self.bitmap=@_iconbitmap ? @_iconbitmap.bitmap : nil
  end
  
  def setSpeciesBitmapDex(species,female=false,form=0,shiny=false,shadow=false,back=false,egg=false,box=false,color=0)
    @_iconbitmap.dispose if @_iconbitmap
    @_iconbitmap=pbLoadSpeciesBitmap(species,female,form,shiny,shadow,back,egg,box,color)
    self.bitmap=@_iconbitmap ? @_iconbitmap.bitmap : nil
  end
...
=begin
  def update
    ...
    @spriteYExtra=0
    # Pokémon sprite bobbing while Pokémon is selected
    if ((@frame/6).floor&1)==1 && @selected==1 # When choosing commands for this Pokémon # Was 10
      @spriteYExtra=2
    end
    self.x=@spriteX
    self.y=@spriteY
...
class PokemonIconSprite < SpriteWrapper
  ...
  def update
    ...
    frameskip=15 if @pokemon && @pokemon.hp<=(@pokemon.totalhp/3)
...
def pbPositionPokemonSpriteMirr(sprite,left,top)
