  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    if minlength==0
      @sprites["helpwindow"]=Window_UnformattedTextPokemon.newWithSize(
         ...
         32+64,Graphics.height-96,Graphics.width-64,96,@viewport
      )
    else
      @sprites["helpwindow"]=Window_UnformattedTextPokemon.newWithSize(
    ...
    when 1   # Player
      if $PokemonGlobal
        ...
        if meta
          ...
          @sprites["shadow"].setBitmap("Graphics/UI/"+getDarkModeFolder+"/Naming/icon_shadow")
    ...
    when 3   # Storage box
      ...
      @sprites["subject"].setBitmap("Graphics/UI/"+getDarkModeFolder+"/Naming/icon_storage")
...
=end
  ...
  @@Characters0=[ # Latin
    ...
    [("ΑΒΓΔΕΖΗΘΙΚΛΜΪ"+"ΝΞΟΠΡΣΤΥΦΧΨΩΫ"+"αβγδεζηθικλμϊ"+"νξοπρστυφχψωϋ"+"ΆΈΉΊΌΎΏάέήίόύ").scan(/./),_INTL("greek")],
    [(",.:;!?ß€£♂♀§µ"+"\"'()<>[]¿,¥®©"+"~@#%*&$ÿςώ   "+"+-=^_/\\|ΐΰ   "+"0123456789   ").scan(/./),_INTL("other")],
  ]
...
=end
  ...
  class NameEntryCursor
    def initialize(viewport)
      ...
      @cursortype=0
      @cursor1=AnimatedBitmap.new("Graphics/UI/"+getAccentFolder+"/NamingCursor1")
      @cursor2=AnimatedBitmap.new("Graphics/UI/"+getAccentFolder+"/NamingCursor2")
      @cursor3=AnimatedBitmap.new("Graphics/UI/"+getAccentFolder+"/NamingCursor3")
      @cursorPos=0
      updateInternal
    end
    ...
    def updateCursorPos
      ...
      if value==PokemonEntryScene2::MODE1 # Upper case
        ...
        @sprite.x=52+32*(value%PokemonEntryScene2::ROWS)+64
  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    @bitmaps=[
       AnimatedBitmap.new("Graphics/UI/"+getDarkModeFolder+"/Naming/overlay_tab_1"),
       AnimatedBitmap.new("Graphics/UI/"+getDarkModeFolder+"/Naming/overlay_tab_2"),
       AnimatedBitmap.new("Graphics/UI/"+getDarkModeFolder+"/Naming/overlay_tab_3"),
       AnimatedBitmap.new("Graphics/UI/"+getDarkModeFolder+"/Naming/overlay_tab_4")
    ]
    ...
    @sprites["bg"].setBitmap("Graphics/UI/"+getDarkModeFolder+"/Naming/bg")
    case subject
    when 1   # Player
      if $PokemonGlobal
        meta=pbGetMetadata(0,MetadataPlayerA+$PokemonGlobal.playerID)
    ...
    when 3   # Storage box
      ...
      @sprites["subject"].altcharset="Graphics/UI/"+getDarkModeFolder+"/Naming/icon_storage"
    ...
    @sprites["controls"].setBitmap("Graphics/UI/"+getDarkModeFolder+"/Naming/overlay_controls")
  ...
  def pbDoUpdateOverlay2
    ...
    overlay.clear
    modeIcon=[["Graphics/UI/"+getDarkModeFolder+"/Naming/icon_mode",44+@mode*62+64,120,@mode*60,0,60,44]]
  ...
  def pbChangeTab(newtab=@mode+1)
    ...
    @sprites["toptab"].x=@sprites["bottomtab"].x-64
    @sprites["bottomtab"].x=tempx+64
