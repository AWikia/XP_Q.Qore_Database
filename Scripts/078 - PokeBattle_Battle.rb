...
module PokeBattle_BattleCommon
  def pbStorePokemon(pokemon)
    ...
    # changed added - https://reliccastle.com/resources/176/
    if swap
      pbDisplayPaused(_INTL("{2} was added to {1}'s party!",$Trainer.name,pokemon2.name))
    end
    # changed end
  end
  ...
  def pbThrowPokeBall(idxPokemon,ball,rareness=nil,showplayer=false)
    ...
    elsif $game_switches[172]
      @scene.pbThrowAndDeflect(ball,1)
      pbDisplay(_INTL("You can't catch any species during a Museum Mission!"))
    else
...
################################################################################
  def initialize(scene,p1,p2,player,opponent)
    $inbattle=true  # This is needed or else SBC on other forms will try to learn moves
    if p1.length==0
...
# Checks whether an item can be removed from a Pokémon.
  def pbIsUnlosableItem(pkmn,item)
    ...
    if isConst?(pkmn.ability,PBAbilities,:RKSSYSTEM)
      ...
      # FLINT Memories
      memories+=[:MAGICMEMORY,:DOOMMEMORY,:JELLYMEMORY,:SHARPENERMEMORY,
                 :LAVAMEMORY,:WINDMEMORY,:LICKMEMORY,:BOLTMEMORY,:HERBMEMORY,
                 :CHLOROPHYLLMEMORY,:GUSTMEMORY,:SUNMEMORY,:MOONMEMORY,
                 :MINDMEMORY,:HEARTMEMORY,:BLIZZARDMEMORY,:GASMEMORY,
                 :GLIMSEMEMORY]
      for i in memories
        return true if isConst?(item,PBItems,i)
      end
...
################################################################################
  ...
  def pbCanChooseMove?(idxPokemon,idxMove,showMessages,sleeptalk=false)
    ...
    if thispkmn.effects[PBEffects::ThroatChop]>0 && thismove.isSoundBased? # changed added
      if showMessages
        pbDisplayPaused(_INTL("The effects of Throat Chop prevent {1} from using certain moves!",thispkmn.pbThis,thismove.name))
      end
      return false
    end
    if @field.effects[PBEffects::Gravity]>0 && thismove.unusableInGravity?
      if showMessages
        pbDisplayPaused(_INTL("{1} can't use {2} because of gravity!",thispkmn.pbThis,thismove.name))
      end
      return false
    end
    ...
    if thispkmn.effects[PBEffects::Khleri]>0 && thismove.basedamage>0
      if showMessages
        pbDisplayPaused(_INTL("{1} can't use {2} after the khleri!",thispkmn.pbThis,thismove.name))
      end
      return false
    end
    ...
    # Trummet Spirit
    if opp1.hasWorkingAbility(:TRUMMETSPIRIT) && !thispkmn.hasMoldBreaker(opp1)
      if thismove.id==thispkmn.lastMoveUsed
        if showMessages
    ...
    elsif opp2.hasWorkingAbility(:TRUMMETSPIRIT) && !thispkmn.hasMoldBreaker(opp2)
      if thismove.id==thispkmn.lastMoveUsed
        if showMessages
    ...
    if thismove.function==0x267 && # Stuff Cheeks
       !pbIsBerry?(thispkmn.item)
      if showMessages
        pbDisplayPaused(_INTL("{1} can't use that move because it doesn't have any berry!",thispkmn.pbThis))
      end
      return false
    end
  ...
  def pbPriority(ignorequickclaw=false,log=false)
    ...
    for i in 0...4
      ...
      if !ignorequickclaw && @choices[i][0]==1 # Chose to use a move
        ...
        if !quickclaw[i] && !quickdraw[i] && 
            @battlers[i].hasWorkingAbility(:QUICKDRAW)
          if pbRandom(10)<2
            quickdraw[i]=true
            pbDisplayBrief(_INTL("{1}'s {2} let it move first!",
               @battlers[i].pbThis,PBAbilities.getName(@battlers[i].ability)))
          end
        end
        if !quickclaw[i] &&  !quickdraw[i] &&
    ...
    for i in 0...4
      ...
      if @choices[i][0]==1 # Chose to use a move
        ...
        pri+=1 if @battlers[i].effects[PBEffects::SixtopiaP]
        pri+=1 if @battlers[i].hasWorkingAbility(:PRANKSTER) &&
                  @choices[i][2].pbIsStatus?
        pri+=1 if @battlers[i].hasWorkingAbility(:TRIAGE) && 
                  @choices[i][2].isHealingMove?
        pri+=1 if @battlers[i].hasWorkingAbility(:SOUNDTRACK) && 
                  @choices[i][2].isSoundBased?
        pri+=1 if @battlers[i].hasWorkingAbility(:SINISTRO) && 
                  @choices[i][2].isContactMove?
        pri+=1 if @battlers[i].hasWorkingAbility(:SONIKO) &&
                  !@choices[i][2].pbIsStatus?
          ...
          for i in 1...temp.length
            ...
            if quickclaw[temp[i]] || quickdraw[temp[i]]
              ...
              if @field.effects[PBEffects::TrickRoom]>0 &&
                 !@battlers[temp[i]].hasWorkingAbility(:MORFAT)
                cmp=(speeds[temp[i]]>speeds[temp[i-1]]) ? 1 : -1
              else
                cmp=(speeds[temp[i]]>speeds[temp[i-1]]) ? -1 : 1
              end
...
################################################################################
  ...
  def pbRecallAndReplace(index,newpoke,newpokename=-1,batonpass=false,moldbreaker=false)
    neutralizinggas=false; user=@battlers[index]
    if isConst?(@battlers[index].ability,PBAbilities,:NEUTRALIZINGGAS) && !@battlers[index].isFainted?
      if ((isConst?(@battlers[index].pbPartner.ability,PBAbilities,:NEUTRALIZINGGAS) && 
           !@battlers[index].pbPartner.isFainted?) ||
          (isConst?(@battlers[index].pbOpposing1.ability,PBAbilities,:NEUTRALIZINGGAS) && 
           !@battlers[index].pbOpposing1.isFainted?) ||
          (isConst?(@battlers[index].pbOpposing2.ability,PBAbilities,:NEUTRALIZINGGAS) && 
           !@battlers[index].pbOpposing2.isFainted?))
      ...
      if !neutralizinggas
        ...
        for j in pbPriority
          next if user==j || j.pbHasType?(:GAS)
...
# Uses an item on an active Pokémon.
  ...
  def pbEnemyUseItem(item,battler)
    ...
    if isConst?(item,PBItems,:SWEETCANDY)
      battler.pbRecoverHP(10,true)
      pbDisplay(_INTL("{1}'s HP was restored.",battler.pbThis))
    elsif isConst?(item,PBItems,:POTION) ||
          isConst?(item,PBItems,:SOURCANDY)
      battler.pbRecoverHP(20,true)
      pbDisplay(_INTL("{1}'s HP was restored.",battler.pbThis))
    elsif isConst?(item,PBItems,:SPICYCANDY)
      battler.pbRecoverHP(40,true)
      pbDisplay(_INTL("{1}'s HP was restored.",battler.pbThis))
      ...
      pbDisplay(_INTL("{1}'s HP was restored.",battler.pbThis))
    elsif isConst?(item,PBItems,:MEGAPOTION)
...
################################################################################
  ...
  def pbOnActiveAll
    ...
    priority=pbPriority
    for i in priority
      i.pbAbilitiesOnSwitchIn(true) if isConst?(i.ability,PBAbilities,:NEUTRALIZINGGAS)
    end
    for i in priority
  ...
  def pbOnActiveOne(pkmn,onlyabilities=false,moldbreaker=false)
    ...
    if !onlyabilities
      ...
      # Sixtopia
      if pkmn.effects[PBEffects::Sixtopia]
        PBDebug.log("[Lingering effect triggered] #{pkmn.pbThis}'s Sixtopia")
        pbCommonAnimation("Sixtopia",pkmn,nil)
        pbDisplayPaused(_INTL("The Sixtopia came true for {1}!",pkmn.pbThis(true)))
        pkmn.effects[PBEffects::SixtopiaP]=true
        pkmn.pbRecoverHP(pkmn.totalhp,true)
        pkmn.pbCureStatus(false)
      ...
      if pkmn.pbOwnSide.effects[PBEffects::Spikes]>0 && !pkmn.isAirborne?(moldbreaker)
        if !(pkmn.hasWorkingAbility(:MAGICGUARD) || 
             pkmn.hasWorkingAbility(:SUPERCLEARBODY) ||
...
################################################################################
  def pbAttackPhase
      ...
      if @choices[i.index][0]==2 && !i.effects[PBEffects::SkipTurn]
        ...
        for j in priority
          ...
          if (pbChoseMoveFunctionCode?(j.index,0x88) ||      # Pursuit
              pbChoseMoveFunctionCode?(j.index,0x187) ||     # Douze Crayon
              pbChoseMoveFunctionCode?(j.index,0x194) ||     # Salazire
              pbChoseMoveFunctionCode?(j.index,0x294)) &&    # Blizzardi Chase
             !j.hasMovedThisRound?
    ...
    for i in priority
      ...
      elsif pbChoseMoveFunctionCode?(i.index,0x211) # Shell Trap
        pbDisplay(_INTL("{1} set a shell trap!",i.pbThis))
      elsif pbChoseMoveFunctionCode?(i.index,0x220) # Beak Blast # changed added
...
#        pbDisplay(_INTL("The sunlight is strong."))
        if pbWeather==PBWeather::SUNNYDAY
          ...
          for i in priority
            if i.hasWorkingAbility(:SOLARPOWER) && !i.hasWorkingItem(:UTILITYUMBRELLA)
            ...
            if i.pbHasType?(:SUN) && !i.isFainted?
              hpgain=i.pbRecoverHP((i.totalhp/8).floor,true)
              pbDisplay(_INTL("{1} was healed by the sunlight!",i.pbThis)) if hpgain>0
            end
          end
        end
      end
        ...
        if pbWeather==PBWeather::HAIL
          ...
          for i in priority
            ...
            if !i.pbHasType?(:ICE) && !i.pbHasType?(:MAGIC) && !i.pbHasType?(:DOOM) &&
              ...
              if i.hasWorkingAbility(:ICEFACE)  && i.form==1
                i.effects[PBEffects::IceFace]=true
                i.form=0
                i.pbUpdate(true)
                @battle.scene.pbChangePokemon(i,i.pokemon)
                pbDisplay(_INTL("{1} regained its ice face!",i.pbThis))
              end
              if i.isFainted?
                return if !i.pbFaint
              end
...
#        pbDisplay(_INTL("The bolty cauldron still emergences"))
    ...
    for i in battlers   # not priority
      ...
      if i.effects[PBEffects::FutureSight]>0
        ...
        if i.effects[PBEffects::FutureSight]==0
          ...
          elsif i.hasWorkingAbility(:CLOCKAPIA)
            if i.pbIncreaseStatWithCause(PBStats::ACCURACY,1,i,PBAbilities.getName(i.ability))
    ...
    for i in priority
      ...
      # Pastry Sun
      if i.hasWorkingAbility(:PASTRYSUN) &&
         (pbWeather==PBWeather::SUNNYDAY ||
         pbWeather==PBWeather::HARSHSUN)  && 
         !i.hasWorkingItem(:UTILITYUMBRELLA)
        PBDebug.log("[Ability triggered] #{i.pbThis}'s Pastry Sun")
        hpgain=i.pbRecoverHP((i.totalhp/16).floor,true)
        pbDisplay(_INTL("{1}'s {2} restored its HP a little!",i.pbThis,PBAbilities.getName(i.ability))) if hpgain>0
      end
      # Dry Skin
    ...
    for i in 0...2
      if sides[i].effects[PBEffects::SeaOfFire]>0 &&
        ...
        for j in priority
          next if (j.index&1)!=i
          next if j.pbHasType?(:FIRE) || j.hasWorkingAbility(:MAGICGUARD) || j.hasWorkingAbility(:SUPERCLEARBODY)
          @scene.pbDamageAnimation(j,0)
          hploss=j.pbReduceHP((j.totalhp/8).floor)
#          j.temperature2= [[3,5][rand(2)],3,2,1,-1,-2][rand(6)] if rand(10) < 5
    ...
    # Glimmy Galaxy (Common Animation)
    ...
    for i in priority
      ...
      if pbTerrain==PBBattleTerrains::VOLCANIC && !i.isAirborne?
        ...
        if i.pbHasType?(:HERB) || i.pbHasType?(:CHLOROPHYLL)
          ...
          i.pbReduceHP((i.totalhp/16).floor) if !(i.hasWorkingAbility(:MAGICGUARD) || i.hasWorkingAbility(:SUPERCLEARBODY))
        end
        pbDisplay(_INTL("{1} was hurt by the Volcanic Terrain.",i.pbThis))
        i.temperature2= [2,1][rand(2)] if rand(100) < 70
        if i.isFainted?
          return if !i.pbFaint
        end
      end
      ...
      if @field.effects[PBEffects::GlimmyGalaxy]>0 &&
        ...
        @scene.pbDamageAnimation(i,0)
        i.temperature2= -2 if rand(100) < 20
        if (i.pbHasType?(:HERB) || i.pbHasType?(:CHLOROPHYLL)) && !i.pbHasType?(:SUN)
          i.pbReduceHP((i.totalhp/16).floor)
        elsif i.pbHasType?(:SUN)  && !(i.pbHasType?(:HERB) || i.pbHasType?(:CHLOROPHYLL))
          i.pbReduceHP((i.totalhp/4).floor)
        else
          i.pbReduceHP((i.totalhp/8).floor)
        end
        pbDisplay(_INTL("{1} was hurt by the Glimmy Galaxy.",i.pbThis))
        if i.isFainted?
          return if !i.pbFaint
        end
      end
    ...
    for i in priority
      next if i.isFainted?
      if i.effects[PBEffects::LeechSeed]>=0 && !((i.hasWorkingAbility(:MAGICGUARD)) || (i.hasWorkingAbility(:SUPERCLEARBODY)))
    ...
    for i in priority
      ...
      if i.status==PBStatuses::POISON
        ...
        if i.hasWorkingAbility(:POISONHEAL) || i.pbHasType?(:MOON)
...
#            pbCommonAnimation("BadPoison",i,nil)
          end
        else
          if !((i.hasWorkingAbility(:MAGICGUARD)) || (i.hasWorkingAbility(:SUPERCLEARBODY)))
            ...
            if i.statusCount==0
              ...
              if i.pbHasType?(:HERB) || i.pbHasType?(:CHLOROPHYLL)
              i.pbReduceHP(((i.totalhp*i.effects[PBEffects::Toxic])/32).floor)
              else
      ...
      if i.status==PBStatuses::BURN
        if i.pbHasType?(:SUN) && !i.isFainted?
          i.pbRecoverHP((i.totalhp/16).floor,true)
          pbDisplay(_INTL("{1} is healed by burn!",i.pbThis))
#          pbCommonAnimation("Burn",i,nil)
        elsif !((i.hasWorkingAbility(:MAGICGUARD)) || (i.hasWorkingAbility(:SUPERCLEARBODY)))
    ...
    for i in priority
      next if i.isFainted?
      if i.effects[PBEffects::Curse] && !(i.hasWorkingAbility(:MAGICGUARD) || i.hasWorkingAbility(:SUPERCLEARBODY))
    ...
    for i in priority
      next if i.isFainted?
      if i.effects[PBEffects::Khleri]>0
        i.effects[PBEffects::Khleri]-=1
        if i.effects[PBEffects::Khleri]==0
          pbDisplay(_INTL("{1}'s khleri wore off!",i.pbThis))
          PBDebug.log("[End of effect] #{i.pbThis} is no longer khlered")
        end 
      end
    end
    ...
    # Throat Chop # changed added
    for i in priority
      next if i.isFainted?
      if i.effects[PBEffects::ThroatChop]>0
        i.effects[PBEffects::ThroatChop]-=1
        if i.effects[PBEffects::ThroatChop]==0
          pbDisplay(_INTL("{1}'s Throat Chop wore off!",i.pbThis))
    ...
    # Electromania
    for i in 0...2
      if sides[i].effects[PBEffects::Electromania]>0
        sides[i].effects[PBEffects::Electromania]-=1
        if sides[i].effects[PBEffects::Electromania]==0
          pbDisplay(_INTL("The Magnetic Electric Field faded from your team!")) if i==0
          pbDisplay(_INTL("The Magnetic Electric Field faded from the opposing team!")) if i==1
          PBDebug.log("[End of effect] Electromania ended on the player's side") if i==0
          PBDebug.log("[End of effect] Electromania ended on the opponent's side") if i==1
        end
      end
    end
    # Fierymania
    for i in 0...2
      if sides[i].effects[PBEffects::Fierymania]>0
        sides[i].effects[PBEffects::Fierymania]-=1
        if sides[i].effects[PBEffects::Fierymania]==0
          pbDisplay(_INTL("The Fiery Vortex Field faded from your team!")) if i==0
          pbDisplay(_INTL("The Fiery Vortex Field faded from the opposing team!")) if i==1
          PBDebug.log("[End of effect] Fierymania ended on the player's side") if i==0
          PBDebug.log("[End of effect] Fierymania ended on the opponent's side") if i==1
        end
      end
    end
    # Toxic Swamp
    for i in 0...2
      if sides[i].effects[PBEffects::ToxicSwamp]>0
        sides[i].effects[PBEffects::ToxicSwamp]-=1
        if sides[i].effects[PBEffects::ToxicSwamp]==0
          pbDisplay(_INTL("The Swamp of Poison faded from your team!")) if i==0
          pbDisplay(_INTL("The Swamp of Poison faded from the opposing team!")) if i==1
          PBDebug.log("[End of effect] Toxic Swamp ended on the player's side") if i==0
          PBDebug.log("[End of effect] Toxic Swamp ended on the opponent's side") if i==1
        end
      end
    end
    # Brainologic
    for i in 0...2
      if sides[i].effects[PBEffects::Brainologic]>0
        sides[i].effects[PBEffects::Brainologic]-=1
        if sides[i].effects[PBEffects::Brainologic]==0
      ...
      for i in priority
        ...
        if i.hasWorkingItem(:ROOMSERVICE)
          if i.pbCanReduceStatStage?(PBStats::SPEED,i,false,self)
            pbCommonAnimation("UseItem",i,nil)
            i.pbReduceStatWithCause(PBStats::SPEED,1,i,PBItems.getName(i.item))
            i.pbConsumeItem
          end
        end
      end
    ...
    for i in priority
      ...
      if i.pbHasType?(:GAS) && i.pokemon.corrosiveGas
        i.pokemon.corrosiveGas=false
        pbDisplay(_INTL("{1}'s {2} is no longer corroded!",i.pbThis,PBItems.getName(i.item)))
      end
      # Higher Temperatures
      # Burning
      if i.highTemp? && i.turncount>0 && i.pbCanBurn?(nil,false) && 
       ( rand(100) < 5 ||
        (rand(25) < 5 && i.tooHighTemp?)
        )
        PBDebug.log("[#{i.pbThis} burned itself due to overheating]")
        i.pbBurn(nil,_INTL("{1}'s increased temperature made itself burn!",i.pbThis))
      end
      # Switching out
      if i.turncount>0 &&
         i.tempHH? &&
         rand(100) < 98 &&
         ( (pbCanChooseNonActive?(i.index) &&
         !pbAllFainted?(pbParty(i.index))) ) # ||
     #    !opponent # wild
        PBDebug.log("[#{i.pbThis} switched out due to overheating]")
        pbDisplay(_INTL("{1} is overheating. It must switch out!",i.pbThis))
        @choices[i.index][0] = -1
        @choices[i.index][1] = -1
        @choices[i.index][2] = -1
        @choices[i.index][3] = -1
      ...
      if i.turncount>0 &&
         ...
         !pbAllFainted?(pbParty(i.index))) ) # ||
     #    !opponent # wild
        PBDebug.log("[#{i.pbThis} switched out due to overfreezing]")
        pbDisplay(_INTL("{1} is overfreezing. It must switch out!",i.pbThis))
        @choices[i.index][0] = -1
        @choices[i.index][1] = -1
        @choices[i.index][2] = -1
        @choices[i.index][3] = -1
      ...
      # Octolock
      # A Pokémon's turncount is 0 if it became active after the beginning of a round
      if i.turncount>0 && i.effects[PBEffects::Octolock]
        if i.pbReduceStat(PBStats::DEFENSE,1,i,false,self,true)
          PBDebug.log("[Effect triggered] #{i.pbThis}'s Octolock")
        end
        if i.pbReduceStat(PBStats::SPDEF,1,i,false,self,true)
          PBDebug.log("[Effect triggered] #{i.pbThis}'s Octolock")
        end
      end
      # Syrup Bomb
      ...
      # Hunger Switch
      if isConst?(i.species,PBSpecies,:MORPEKO) && i.hasWorkingAbility(:HUNGERSWITCH) &&
         i.turncount!=0 && !i.effects[PBEffects::Transform]
        i.form=(i.form+1)%2
        i.pbUpdate(true)
    ...
    for i in priority
      ...
      # Sticky Barb
      if i.hasWorkingItem(:STICKYBARB) && !(i.hasWorkingAbility(:MAGICGUARD) || i.hasWorkingAbility(:SUPERCLEARBODY))
    ...
    for i in 0...4
      ...
      if @battlers[i].effects[PBEffects::Eternamax]>0
        @battlers[i].effects[PBEffects::Eternamax]-=1
        if @battlers[i].effects[PBEffects::Eternamax]==0
          @battlers[i].form=0
          @battlers[i].pbUpdate(true)
          scene.pbChangePokemon(@battlers[i],@battlers[i].pokemon)
          pbDisplay(_INTL("{1}'s Eternamax wore off",@battlers[i].pbThis))
        end
      end
      if @battlers[i].effects[PBEffects::UltraBurst]>0
        @battlers[i].effects[PBEffects::UltraBurst]-=1
        if @battlers[i].effects[PBEffects::UltraBurst]==0
          @battlers[i].form=@battlers[i].effects[PBEffects::UBForm]
          @battlers[i].pbUpdate(true)
          scene.pbChangePokemon(@battlers[i],@battlers[i].pokemon)
    ...
    @field.effects[PBEffects::MagicStorm]=false
    @field.effects[PBEffects::Torchwood]=false
    ...
    when 2, 5
      ...
      if @internalbattle
        ...
        if @opponent
          ...
          if moneylost>0
            pbDisplayPaused(_INTL("{1} paid ${2}\r\nas the prize money...",self.pbPlayer.name,lostmoney.to_s_formatted))  
